...
object WeaveShuffleConf {
  ...
  def fromSparkConf(conf: SparkConf): WeaveShuffleConf = {
    WeaveShuffleConf(
      ...
      numWeavers = conf.getInt("spark.weave.numWeavers", 2),
...
object ShufflePlanConfig {
  def from(conf: WeaveShuffleConf): ShufflePlanConfig = {
    ShufflePlanConfig(
      alpha = conf.alpha,
      beta = conf.beta,
      numBins = conf.numWeavers,
      seed = conf.globalSeed,
      isAssociative = conf.fakePaddingStrategy.toLowerCase == "associative",
      binomialMode = conf.binomialMode
    )
  }
}
