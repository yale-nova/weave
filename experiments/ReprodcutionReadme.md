...
3- Weave does have two manifests, java.manifest.template available at [the config snapshot](http://weave.eastus.cloudapp.azure.com:5555/config_snapshot/), which configures the PAL loader for Gramine  direct mode (Syscall proxying only) and a SGX manifest java.manifest.sgx-template, which is used when SGX is available on the machine. 
